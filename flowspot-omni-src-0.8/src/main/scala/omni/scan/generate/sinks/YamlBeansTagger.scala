...
object YamlBeansTagger extends QueryBundle{
  ...
  def invoke(): Query =
    Query.make(
      ...
      score = 8,
      withStrRep({ cpg =>
        (cpg.call.methodFullName(".*com\\.esotericsoftware\\.yamlbeans\\.YamlReader.*").argument)
      }),
      sinkPattern=".*com\\.esotericsoftware\\.yamlbeans\\.YamlReader.*",
